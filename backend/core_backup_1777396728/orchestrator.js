...
const pattern = detectPattern(convo, msg);
const mode = decideMode(pattern);

const systemPrompt = buildSystemPrompt(convo, userId, userMemory, mode, pattern);

function detectPattern(convo, msg) {
  const lastMessages = convo.slice(-6).map(m => m.content);

  const repeats = lastMessages.filter(t => t === msg).length;

  if (repeats >= 2) return "loop";

  if (/فشلت|fail|again/i.test(msg)) return "failure";

  if (/ضايع|lost|confused/i.test(msg)) return "lost";

  if (/سلام|hi|slm/i.test(msg)) return "avoidance";

  return "normal";
}

function decideMode(pattern) {
  switch (pattern) {
    case "loop":
      return "confront";
    case "failure":
